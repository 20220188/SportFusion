...
const SAVE_FORM = document.getElementById('saveForm'),
    ...
    ESTADO_DEPORTE = document.getElementById('estadoDeporte');

// constantes para el funcionamiento de las graficas.
...
const fillTable = async (form = null) => {
    ...
    if (DATA.status) {
        ...
        DATA.dataset.forEach(row => {
            ...
            TABLE_BODY.innerHTML += `
                <tr>
                    <td><img src="${SERVER_URL}images/deportes/${row.imagen_deporte}" height="50"></td>
...
const openCreate = () => {
    ...
    SAVE_MODAL.show();
    MODAL_TITLE.textContent = 'Crear deporte';
...
const openUpdate = async (id) => {
    ...
    if (DATA.status) {
        ...
        NOMBRE_DEPORTE.value = ROW.nombre_deporte;
        ESTADO_DEPORTE.checked = ROW.estado_retro;
    } else {
        sweetAlert(2, DATA.error, false);
    }
}
...
const openChart = async (id) => {
    ...
    FORM.append('idDeporte', id);
    // Petición para obtener los datos del registro solicitado.
    const DATA = await fetchData(DEPORTE_API, 'readTopProductosxDeporte', FORM);
    ...
    if (DATA.status) {
        ...
        barGraph('chart', productos, unidades, 'Cantidad de productos', 'Top 5 de productos con más elementos en stock');
    } else {
        sweetAlert(4, DATA.error, true);
    }
}
...
const openAllReport = () => {
    // Se declara una constante tipo objeto con la ruta específica del reporte en el servidor.
    const PATH = new URL(`${SERVER_URL}reports/admin/All_productos_deporte.php`);
    // Se abre el reporte en una nueva pestaña.
    window.open(PATH.href);
}